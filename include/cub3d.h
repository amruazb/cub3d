...
/*   Updated: 2024/05/14 20:53:52 by aabashee         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
enum e_dir  //e_side - to define sides in a BMP image: NORTH, SOUTH, EAST, WEST, FLOOR, CEIL
{
	E_N,
	E_E,
	E_S,
...
typedef struct s_point//s_point - a simple struct to represent 2D coordinates (x, y)
{
	double	x;
	double	y;
}	t_point;

typedef struct s_dda_dat //data used during DDA casting
{
	int		dx;
	int		dy;
	int		dx_abs;
...
typedef struct s_img // image information
{
	void	*img;
	char	*addr;
	int		bits_per_pixel;
